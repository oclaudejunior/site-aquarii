...
const ProgramCardsList = ({ showHero = true }: ProgramCardsListProps) => (
  <section id="programas" className="relative bg-background overflow-hidden">
    {showHero && (
      <AnimatedSection>
        <div className="container mx-auto max-w-4xl text-center space-y-5 relative z-10 px-4 pt-20 md:pt-28 pb-12 md:pb-16">
          ...
          >
            ...
            <div className="absolute inset-0 z-0">
              <img
                ...
                width={1920}
                height={1080}
                className="w-full h-full object-cover grayscale brightness-[0.65] scale-100 transition-all duration-[900ms] ease-[cubic-bezier(0.16,1,0.3,1)] group-hover:grayscale-0 group-hover:brightness-90 group-hover:scale-[1.03] group-focus-visible:grayscale-0 group-focus-visible:brightness-90"
              />
            </div>

            {/* Overlays */}
            <div className="absolute inset-0 z-10 bg-background/70 transition-opacity duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] group-hover:bg-background/35 group-focus-visible:bg-background/35" />
            <div className="absolute inset-0 z-10 bg-gradient-to-r from-background/85 via-background/55 to-background/40 pointer-events-none" />
            ...
            <div className="relative z-20 container mx-auto max-w-7xl px-6 md:px-12 py-14 md:py-16 min-h-[220px] md:min-h-[260px] flex flex-col md:flex-row md:items-center gap-8 md:gap-10">
              ...
              <div className="flex-1 md:max-w-[55%]">
                <p className="font-mono text-[10px] md:text-[11px] uppercase tracking-[0.3em] text-primary mb-3 transition-colors duration-500">
                  Pilar {p.num} · {p.chip}
                </p>
                <h3 className="font-heading italic text-3xl md:text-5xl lg:text-6xl text-foreground leading-[1.05] tracking-[-0.02em] transition-transform duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] group-hover:translate-x-2">
                  {p.title}
                </h3>
              </div>

              {/* Right: description */}
              <div className="flex-1 md:max-w-[35%] md:pl-6 md:border-l md:border-[hsl(var(--primary)/0.18)]">
                <p className="font-body text-sm md:text-[15px] leading-relaxed text-foreground/85 line-clamp-4">
                  {p.text}
                </p>
              </div>
              ...
              <div className="md:absolute md:right-12 md:top-1/2 md:-translate-y-1/2 shrink-0">
                <span
                  className="inline-flex items-center justify-center w-12 h-12 md:w-14 md:h-14 rounded-full border border-primary/40 text-primary/80 transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] group-hover:border-primary group-hover:bg-primary group-hover:text-background group-hover:rotate-45"
                  aria-hidden="true"
                >
